/* WebrtcRTCPeerConnection.mm — mac 独立 C ABI 的 PC 业务实现。
 * 照抄自 common/darwin/Classes/FlutterRTCPeerConnection.m，去 Flutter：
 *   - eventSink/FlutterEventChannel/FlutterStreamHandler → WebrtcEventCallback (C ABI)
 *   - FlutterResult → WebrtcResult；FlutterError → WebrtcError
 *   - didOpenDataChannel 里 eventChannel/eventQueue/setStreamHandler 接线删除，
 *     dataChannel 事件由 WebrtcRTCDataChannel 的 webrtcEventCallback 承接
 * 业务逻辑本体(signal/stats/rtp capabilities/codec preference)逐字保留。
 */
...
@implementation WebrtcPlugin (RTCPeerConnection)
...
- (void)peerConnectionGetStatsForTrackId:(nonnull NSString*)trackID
  ...
  if (sender != nil) {
    ...
    result([WebrtcError errorWithCode:@"GetStatsFailed"
...
- (void)peerConnection:(RTCPeerConnection*)peerConnection
  ...
  if (cb) {
    [cb post:@{@"event" : @"signalingState", @"state" : [self stringForSignalingState:newState]}];
  }
}
...
- (void)peerConnection:(RTCPeerConnection*)peerConnection
  ...
  WebrtcEventCallback* cb = peerConnection.webrtcEventCallback;
  if (cb) {
    [cb post:@{@"event" : @"iceGatheringState", @"state" : [self stringForICEGatheringState:newState]}];
  }
}
...
- (void)transceiverSetCodecPreferences:(nonnull NSDictionary*)argsMap
  ...
  if (transcevier == nil) {
    result([WebrtcError errorWithCode:@"transceiverSetCodecPreferencesFailed"
  ...
  if (error) {
    result([WebrtcError errorWithCode:@"SetCodecPreferencesError"
